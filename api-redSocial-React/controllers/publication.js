...
const save = async (req, res) => {
    try {
        ...
        // Si no me llegan datos, dar respuesta negativa
        if (!params.text) {
...
const feed = async (req, res) => {
    ...
    try {
        ...
        // Ahora, en lugar de usar el callback con paginate(), usaremos await y Promise
        const publications = await Publication.find({ user: myFollows.following })
            .populate("user", "-password -role -__v -email")
            .sort("-created_at")
            .skip((page - 1) * itemsPerPage) // saltar las publicaciones de las páginas anteriores
            .limit(itemsPerPage) // limitar la cantidad de publicaciones por página
            .exec(); // ejecutar la consulta de manera sincrónica
